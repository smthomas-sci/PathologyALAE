...
class Summary(TensorBoard):
    ...
    def visualise_progress(self, z_test, test_batch, epoch, n):
        ...
        samples = self.model.generator([z_test[:n]] + [test_batch[1][:n]]).numpy()
...
def initialise_run_directories(run_dir):
    # Create run directories as needed
    ...
    for path in [log_dir, out_dir, weight_dir, fid_dir]:
        if not os.path.exists(path):
            os.mkdir(path)
            print(f"Creating directory:", path)
    return log_dir, out_dir, weight_dir, fid_dir
